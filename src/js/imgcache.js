...
(function ($) {
  ...
  ImgCache.get = (image_url, callback,) => {
    ImgCache.isCached(image_url, (path, success) => {
      if(success) {
        callback(path, 1);
      }
      else {
        ImgCache.cacheFile(image_url, (path) => {
          callback(path, 0);
        }, () => {
          //console.error('could not cache image');
          callback(image_url, -1);
        });
      }
    });
  };

  ImgCache.bg = function($el) {
